...
const float TEAPOT_DISTANCE = 20.0f;
...
bool EngineDemo::Initialize(Engine::MyWindow *window)
{
	...
	for (int i = 0; i < NUM_TEAPOTS; ++i)
	{
		...
		m_teapots[i].SetTransMat(Engine::Mat4::Translation(Engine::Vec3(((i / TEAPOTS_PER_SHADER) * TEAPOT_DISTANCE), 0.0f, ((i%TEAPOTS_PER_SHADER)*TEAPOT_DISTANCE - TEAPOT_DISTANCE))));
...
{
	static Engine::RayCastingOutput s_rco;
	static int s_teapotIndex = -1;
	...
	s_rco = Engine::CollisionTester::FindFromMousePos(Engine::MouseManager::GetMouseX(), Engine::MouseManager::GetMouseY(), 500.0f);
	for (int i = 0; i < NUM_TEAPOTS && s_rco.m_didIntersect; ++i)
	{
		if (s_rco.m_belongsTo == &m_teapots[i]) // if mouse is hovered over a teapot
		{
			if (Engine::MouseManager::IsLeftMouseClicked())
			{
				s_teapotIndex = i;
			}
			else if (Engine::MouseManager::IsLeftMouseReleased() && s_teapotIndex >= 0 && s_teapotIndex < NUM_TEAPOTS)
			{
				Engine::Mat4 transMat = m_teapots[i].GetTransMat();
				m_teapots[i].SetTransMat(m_teapots[s_teapotIndex].GetTransMat());
				m_teapots[s_teapotIndex].SetTransMat(transMat);
				s_rco = Engine::RayCastingOutput();
				s_teapotIndex = -1;

				// for each teapot
				for (int j = 0; j < NUM_TEAPOTS; ++j)
				{
					// compare it to each other teapot
					for (int k = j + 1; k < NUM_TEAPOTS; ++k)
					{
						// and yet again
						for (int l = k + 1; l < NUM_TEAPOTS; ++l)
						{
							// if all three are in a line
							if ((fabsf(m_teapots[j].GetPos().GetX() - m_teapots[k].GetPos().GetX()) < 0.01f) && (fabsf(m_teapots[l].GetPos().GetX() - m_teapots[k].GetPos().GetX()) < 0.01f))
							{
								Engine::Vec3 jCompare = (m_teapots[j].GetMatPtr()->m_diffuseReflectivity + m_lights[j].GetMatPtr()->m_materialColor + m_teapots[j].GetMatPtr()->m_ambientReflectivity) / 3.0f;
								Engine::Vec3 kCompare = (m_teapots[k].GetMatPtr()->m_diffuseReflectivity + m_lights[k].GetMatPtr()->m_materialColor + m_teapots[k].GetMatPtr()->m_ambientReflectivity) / 3.0f;
								Engine::Vec3 lCompare = (m_teapots[l].GetMatPtr()->m_diffuseReflectivity + m_lights[l].GetMatPtr()->m_materialColor + m_teapots[l].GetMatPtr()->m_ambientReflectivity) / 3.0f;

								// if all three are of the same color (TODO: CHANGE TO SHADER!?!??!)
								if (jCompare.EqualsWithinTolerance(kCompare, 0.325f) &&
									kCompare.EqualsWithinTolerance(lCompare, 0.325f) && 
									lCompare.EqualsWithinTolerance(jCompare, 0.325f))
								{
									m_teapots[j].GetMatPtr()->m_diffuseReflectivity= Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));//SetTransMat(Engine::Mat4::Translation(m_teapots[j].GetPos() + Engine::Vec3(0.0f, TEAPOT_DISTANCE, 0.0f)));
									m_teapots[k].GetMatPtr()->m_diffuseReflectivity = Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));//Engine::Vec3(0.0f, 1.0f, 0.0f);//SetTransMat(Engine::Mat4::Translation(m_teapots[k].GetPos() + Engine::Vec3(0.0f, TEAPOT_DISTANCE, 0.0f)));
									m_teapots[l].GetMatPtr()->m_diffuseReflectivity = Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));// Engine::Vec3(0.0f, 1.0f, 0.0f);//SetTransMat(Engine::Mat4::Translation(m_teapots[l].GetPos() + Engine::Vec3(0.0f, TEAPOT_DISTANCE, 0.0f)));
									m_teapots[j].GetMatPtr()->m_ambientReflectivity = Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									m_teapots[k].GetMatPtr()->m_ambientReflectivity = Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									m_teapots[l].GetMatPtr()->m_ambientReflectivity = Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									m_lights[j].GetMatPtr()->m_materialColor= Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									m_lights[k].GetMatPtr()->m_materialColor= Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									m_lights[l].GetMatPtr()->m_materialColor= Engine::MathUtility::Rand(Engine::Vec3(0.0f, 0.0f, 0.0f), Engine::Vec3(1.0f, 1.0f, 1.0f));
									s_score += 3;
									char buffer[50];
									sprintf_s(buffer, 50, "Score: [%d]", s_score);
