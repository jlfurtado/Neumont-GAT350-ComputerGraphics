...
bool EngineDemo::Initialize(Engine::MyWindow *window)
{
	...
	playerGraphicalObject.SetCallback(PCCallback, this);
	...
	for (int i = 0; i < NUM_TEAPOTS; ++i)
	{
		...
		m_teapots[i].SetCallback(EngineDemo::DiffuseCallback, this);
		m_teapots[i].SetBaseColor(Engine::Vec3(1.0f, 1.0f, 1.0f));
		m_teapots[i].SetTintColor(i % 3 == 0 ? Engine::Vec3(0.0f, 0.0f, 0.9f)
		...
		m_lights[i].SetCallback(EngineDemo::PCCallback, this);
		m_lights[i].SetBaseColor(i % 3 == 0 ? Engine::Vec3(1.0f, 1.0f, 1.0f)
									: i % 3 == 1 ? Engine::Vec3(1.0f, 1.0f, 1.0f)
									: Engine::Vec3(1.0f, 0.0f, 0.0f));
		m_lights[i].SetTintColor(i % 3 == 0 ? Engine::Vec3(1.0f, 1.0f, 1.0f)
									: i % 3 == 1 ? Engine::Vec3(1.0f, 1.0f, 1.0f)
									: Engine::Vec3(1.0f, 0.0f, 0.0f));
...
void EngineDemo::PCCallback(Engine::GraphicalObject * instance, void * classInstance)
{
	EngineDemo *pGame = reinterpret_cast<EngineDemo*>(classInstance);

	PassMatrix(instance, classInstance);
	glUniform3f(pGame->debugColorLoc, instance->GetBaseColor().GetR(), instance->GetBaseColor().GetG(), instance->GetBaseColor().GetB());
...
void EngineDemo::DiffuseCallback(Engine::GraphicalObject * instance, void * classInstance)
{
	EngineDemo *pGame = reinterpret_cast<EngineDemo*>(classInstance);
	
	int counter = 0;
	for (int i = 0; i < NUM_TEAPOTS; ++i) { if (instance == &m_teapots[i]) { counter = i; } }

	// pass light location
	glUniform3f(pGame->lightLoc, m_lights[counter].GetPos().GetX(), m_lights[counter].GetPos().GetY(), m_lights[counter].GetPos().GetZ());

	// pass color of material
	glUniform3f(pGame->tintColorLoc, instance->GetBaseColor().GetR(), instance->GetBaseColor().GetG(), instance->GetBaseColor().GetB());

	// pass reflectivity
	glUniform3f(pGame->diffuseColorLoc, instance->GetTintColor().GetR(), instance->GetTintColor().GetG(), instance->GetTintColor().GetB());

	// pass light color
	glUniform3f(pGame->diffuseIntensityLoc, m_lights[counter].GetTintColor().GetR(), m_lights[counter].GetTintColor().GetG(), m_lights[counter].GetTintColor().GetB());

	// pass matrices to shader
	Engine::Mat4 mtw = instance->GetTransMat() * (instance->GetScaleMat() * instance->GetRotMat());
	glUniformMatrix4fv(pGame->modelToWorldMatLoc, 1, GL_FALSE, mtw.GetAddress());

	Engine::Mat4 wtv = player.GetComponentByType<Engine::ChaseCameraComponent>()->GetWorldToViewMatrix();
	glUniformMatrix4fv(pGame->worldToViewMatLoc, 1, GL_FALSE, wtv.GetAddress());

	Engine::Mat4 perspective = pGame->m_perspective.GetPerspective();
	glUniformMatrix4fv(pGame->perspectiveMatLoc, 1, GL_FALSE, perspective.GetAddress());

}

void EngineDemo::DebugCallback(Engine::GraphicalObject * instance, void * classInstance)
{
	EngineDemo *pGame = reinterpret_cast<EngineDemo*>(classInstance);
...
bool EngineDemo::InitializeGL()
{
	...
	matLoc = m_shaderPrograms[1].GetUniformLocation("transform");
...
void EngineDemo::PassMatrix(Engine::GraphicalObject * instance, void * classInstance)
{
	EngineDemo *pGame = reinterpret_cast<EngineDemo*>(classInstance);

	// calc matrix
	Engine::Mat4 wtv = player.GetComponentByType<Engine::ChaseCameraComponent>()->GetWorldToViewMatrix();
	Engine::Mat4 trans = instance->GetTransMat() * ((instance->GetScaleMat()*instance->GetRotMat()));
	Engine::Mat4 finalMat = pGame->m_perspective.GetPerspective() * (wtv * trans);

	// pass matrix
	glUniformMatrix4fv(pGame->matLoc, 1, GL_FALSE, finalMat.GetAddress());
}

void EngineDemo::PassTint(Engine::GraphicalObject * instance, void * classInstance)
{
	EngineDemo *pGame = reinterpret_cast<EngineDemo*>(classInstance);

	glUniform3f(pGame->tintColorLoc, instance->GetTintColor().GetR(), instance->GetTintColor().GetG(), instance->GetTintColor().GetB());
	glUniform1f(pGame->tintIntensityLoc, instance->GetTintIntensity());
}
